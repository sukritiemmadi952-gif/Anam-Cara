...
import { Check, Trash2, X, LogOut } from "lucide-react";
      ...
      {stats && (
        <div className="mt-6 grid grid-cols-2 sm:grid-cols-5 gap-3">
          {[
            ...
            { k: "quizzes_completed", label: "Quizzes done", color: "bg-slate-50 text-slate-800" },
          ].map((s) => (
            <div key={s.k} className={`rounded-2xl ${s.color} px-4 py-3`} data-testid={`admin-stat-${s.k}`}>
              ...
              <div key={r.id} className="rounded-3xl bg-white border border-slate-100 p-5" data-testid={`admin-item-${r.id}`}>
                <div className="flex items-center justify-between text-xs">
                  ...
                  <div className="flex items-center gap-2">
                    {r.flagged && <span className="rounded-full bg-orange-100 text-orange-700 px-2 py-0.5 text-[10px]" data-testid={`admin-flag-${r.id}`}>flagged</span>}
